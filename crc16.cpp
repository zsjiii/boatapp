...
unsigned short CrcCrt( unsigned char * ptr, int Len)
{
    ...
    for(int a=0;a<Len;a++ )
    {
        //CRC = ( CRC << 8 ) ^ CrcCode[( CRC >> 8 ) ^ ptr[a]];
        CRC = ((CRC << 8) & 0xFF00) ^ CrcCode[((CRC >> 8) & 0x00FF) ^ ptr[a]];//��λ����һ��Ҫ�б�֤����
    }
    return( CRC );
}
...
unsigned int CrcCheckNew(unsigned char *Array, unsigned int ArrayLen, unsigned int FirstPlace)
{
    ...
    for(i = FirstPlace; i < ArrayLen + FirstPlace; i++ )
    {
        check = ((check << 8) & 0xff00) ^ CrcCode[((check >> 8) & 0x00ff) ^
            Array[i]]; //��λ����һ��Ҫ�б�֤����
    }
...
//unsigned short  ModBusCrcData(BYTE *buffer, unsigned short len)
unsigned short  ModBusCrcData(unsigned char *buffer, unsigned short len)
{
	//unsigned short  wcrc = 0XFFFF;//16λcrc�Ĵ���Ԥ��
	unsigned short  uIndex ;//����
	unsigned char b_CRC_L = 0xFF;//��������
	unsigned char b_CRC_H = 0xFF;
	while(len--)
	{
		uIndex = b_CRC_H^ *buffer++;
		b_CRC_H = b_CRC_L^auchCRCHi[uIndex];
		b_CRC_L = auchCRCLo[uIndex];
	}
	return ((b_CRC_H << 8) | b_CRC_L);
}

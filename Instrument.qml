...
Item {
    ...
    // 顶层圆弧线宽
    property int top_lineWidth: parent.width/50
    // 顶层圆弧颜色
    property color top_backgroundColor: "lightgreen"
    // 顶层圆弧半径 开始角度 结束角度
    property int top_r: btm_r-10
    ...
    //指针参数
    property real needleLength: top_r
    ...
    Canvas {
        ...
        onPaint: {
            ...
            var neddle_y = (carItem.height)+(carItem.top_r-carItem.top_lineWidth+carItem.dial_addR)*Math.sin(carItem.top_endAngle/180*Math.PI);
            ...
            neddle_y = (carItem.height)+((carItem.top_r-carItem.top_lineWidth+carItem.dial_addR)/10)*Math.sin((carItem.top_endAngle-45)/180*Math.PI);
            ctx.lineTo(neddle_x,neddle_y);
